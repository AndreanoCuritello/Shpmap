package ru.r8digitaltech7.onlylemi.mapview.library;
...
import ru.r8digitaltech7.onlylemi.mapview.library.layer.MapBaseLayer;
import ru.r8digitaltech7.onlylemi.mapview.library.layer.MapLayer;
...
public class MapView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void loadMap(final Picture picture) {
        ...
        Thread loadThread = new Thread(new Runnable() {
            ...
            public void run() {
                if (picture != null) {
                    Log.v(TAG, "loadMap : new Thread");
                    if (mapLayer == null) {
                        Log.v(TAG, "mapLayer == null");
                        mapLayer = new MapLayer(MapView.this);
                        // add map image layer
                        Log.v(TAG, "add map image layer");
                        layers.add(mapLayer);
                        Log.v(TAG, "added map image layer");
                    }
                    Log.v(TAG, "set picture");
                    mapLayer.setImage(picture);
                    Log.v(TAG, "picture setted");
                    if (mapViewListener != null) {
                        // load map success, and callback
                        mapViewListener.onMapLoadSuccess();
                    }
                    isMapLoadFinish = true;
                    Log.v(TAG, "begin refresh");
